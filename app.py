...
st.markdown("""
AI-powered tool for predicting neonatal risk using maternal and birth data.  
Designed for **PHC workers, hospitals, and public health systems**.
""")
...
if input_mode == "Manual Entry":
    ...
    if page == "Clinical Risk Assessment":
        ...
        st.subheader(" Neonatal Risk Assessment (Manual Entry)")

        col1, col2 = st.columns(2)

        with col1:
            v012 = st.text_input("Mother's Age", "25")
            ...
            bord = st.text_input("Birth Order", "1")

        with col2:
            b11 = st.text_input("Birth Interval (months)", "24")
            m15 = st.selectbox("Place of Delivery", ["Home","PHC","Hospital/Clinic"])
            m14 = st.text_input("ANC Visits", "4")
            m1 = st.selectbox("Tetanus Protection", ["Yes","No"])
            b0 = st.selectbox("Multiple Birth", ["No","Yes"])
            m19 = st.text_input("Birth Weight (grams)", "3000")
        ...
        if st.button(" Predict Risk"):
            ...
            try:
                ...
                shap_single_plot(model, df, features)

            except Exception as e:
                st.error(f"Error: {e}")

    else:
        st.info("Manual entry only works for Clinical Risk Assessment module.")
...
# ============================
    ...
    if uploaded_file:
        ...
        if page == "Clinical Risk Assessment":
            ...
            st.subheader("🩺 Individual Case")

            st.dataframe(df.head())

            idx = st.number_input("Select Patient Row", 0, len(df)-1, 0)
            ...
            shap_single_plot(model, case[features], features)
