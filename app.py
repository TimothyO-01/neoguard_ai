...
# ============================
# DATABASE LOGGING (NEW - ADDED ONLY)
# ============================
def log_to_db(df):
    conn = sqlite3.connect("neoguard.db")
    df.to_sql("records", conn, if_exists="append", index=False)
    conn.close()

def audit_log(df):
    conn = sqlite3.connect("neoguard.db")
    df.to_sql("audit_logs", conn, if_exists="append", index=False)
    conn.close()
...
def generate_pdf(pid, result, prob, advice, factors):
    buffer = BytesIO()
...
if input_mode == "Manual Entry":
    ...
    if page != "Clinical Risk Assessment":
        ...
        with col1:
            patient_id = st.text_input("Patient ID", placeholder="Input the Patient ID")   
        ...
        with col1:
            interval = st.text_input("Birth Interval (months)", placeholder="e.g. 12")
            place = st.selectbox("Delivery Place", ["Home","PHC","Hospital"])
            anc = st.text_input("Antenatal Visits", placeholder="e.g. 4")
        ...
        with col1:
            weight = st.text_input("Birth Weight (grams)", placeholder="e.g. 2500")
        ...
        if st.button("Predict Risk"):
            ...
            if not patient_id:
                patient_id = "AUTO_" + str(pd.Timestamp.now().value)[-6:]
            ...
            st.download_button(
                "Download Report",
                pdf,
                f"NeoGuard_Report_{patient_id}.pdf",
                "application/pdf"
            )
...
# ============================
    ...
    if uploaded_file is not None:
        ...
        if "PatientID" not in raw_df.columns:
            raw_df["PatientID"] = [f"AUTO_{i+1}" for i in range(len(raw_df))]
